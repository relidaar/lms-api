...
class CourseCrudTests(APITestCase):
    ...
    def setUp(self) -> None:
        ...
        self.course_detail_url = reverse('course-detail', kwargs={'uuid': self.course.uuid})
    ...
    def test_get_valid_single_course(self):
        ...
        course = Course.objects.get(uuid=self.course.uuid)
    ...
    def test_create_invalid_course(self):
        ...
        self.client.post(self.login_url, self.superuser_login_data)
        response = self.client.post(self.course_list_url)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_valid_update_course_details(self):
        ...
        put_response = self.client.put(self.course_detail_url, data)
        patch_response = self.client.patch(self.course_detail_url, {'code': 'course5'})
        self.assertEqual(put_response.status_code, status.HTTP_200_OK)
        self.assertEqual(patch_response.status_code, status.HTTP_200_OK)
    ...
    def test_invalid_update_course_details(self):
        ...
        self.client.post(self.login_url, self.superuser_login_data)
        response = self.client.put(self.course_detail_url)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
